...
#define DIST_UNIFORM_SORT
...
#elif defined(DIST_UNIFORM_SORT)
        {
            ...
            for (int i = 0; i < KCAS_MAXK; ++i)
            {
                ...
                if (ix > 0 && ix[i] <= ix[i - 1])
                    ix[i] = ix[i - 1] + 1;
                if (ix[i] - (MAXKEY - 1) > maxover)
                    maxover = ix[i] - (MAXKEY - 1);
            }
            if (maxover > 0)
            {
        ...
        for (int i = 0; i < KCAS_MAXK; ++i)
        {
            ...
            if (i > 0 && ix[i] <= ix[i - 1])
            {
                cout << "FAILURE: ix[" << i - 1 << "]=" << ix[i - 1] << " and ix[" << i << "]=" << ix[i] << endl;
                exit(-1);
            }
        }
//        return true;
...
void trial()
{
    ...
    while (running.load() > 0 && elapsedMillisNapping < 200)
    {                            // nap for up to 200ms
